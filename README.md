...
|---|---|---|---|---|---|---|
| **LIGHT (low-memory)** | GPU runtime + RAM 8 GB / 4 core | `Qwen2.5-0.5B-Instruct` | 300 VN Alpaca | 500 UltraFeedback | fastest | Khi cần chắc chạy, ít RAM, tránh lỗi kernel nặng |
...
> T4 và BigGPU dùng **cùng notebook source** — đổi giữa hai tier này bằng `COMPUTE_TIER` trong `.env`. LIGHT là đường chạy riêng tối giản qua `make pipeline-lite` hoặc `colab/Lab22_DPO_Light.ipynb`.

> **VRAM math quan trọng:** DPO chấm mỗi câu dưới *cả* policy và reference. Với PEFT/LoRA, TRL **không** nạp model thứ 2 -- nó giữ 2 LoRA adapter trên cùng base 4-bit: adapter train bắt đầu từ SFT và adapter reference đóng băng ở checkpoint SFT ban đầu. VRAM cao hơn SFT là do **2 forward pass + giữ cả chosen lẫn rejected** trong batch (~1.5-2x activation memory của SFT), *không* phải vì 2 bản base weights. Đó là lý do T4 tier dùng 3B (không 7B) và BigGPU tier yêu cầu A100/L4.

---

## Quick Start — LIGHT (8 GB RAM / 4 core)

Đây là đường chạy nhẹ nhất nếu bạn ưu tiên **ít lỗi, ít RAM, chạy nhanh** hơn điểm benchmark lớn:

```bash
bash setup-laptop.sh
LOW_MEM=1 COMPUTE_TIER=LIGHT make pipeline-lite
make verify   # sau khi điền submission/REFLECTION.md
```

Colab: mở `colab/Lab22_DPO_Light.ipynb`, hoặc chạy setup với `LOW_MEM=1`.

LIGHT dùng `transformers + peft + trl` trực tiếp với model 0.5B, LoRA rank 8, `max_length=384`, `dataloader_num_workers=0`, `dataset_num_proc=1`, và bỏ NB5/NB6 nặng mặc định.
...
Chỉ dùng hướng này khi bạn muốn chạy 7B/full lab và có đủ RAM/disk. Nếu máy là A100 nhưng RAM hệ thống chỉ 8 GB hoặc muốn ổn định nhanh, dùng `make pipeline-lite` ở trên.
...
**Colab variant:** `colab/Lab22_DPO_Light.ipynb`, `colab/Lab22_DPO_T4.ipynb`, and `colab/Lab22_DPO_BigGPU.ipynb` are launchable `.ipynb` files. LIGHT is the low-memory path; T4/BigGPU mirror the Jupytext sources.
...
| Unsloth + TRL version mismatch | Pin: `unsloth[colab-new]==2026.6.9 trl==0.19.1`. Nếu lỗi sau Unsloth update, cài lại đúng cặp này |
...
| A100 nhưng RAM 8 GB / 4 core vẫn lỗi hoặc quá chậm | Chạy `LOW_MEM=1 COMPUTE_TIER=LIGHT make pipeline-lite` hoặc mở `colab/Lab22_DPO_Light.ipynb` |
